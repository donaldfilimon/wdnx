...
def require_query_params(*params):
    def decorator(fn):
        ...
        def wrapper(*args, **kwargs):
            ...
            if missing:
                return jsonify({'error': 'Missing query parameters', 'missing': missing}), 400
...
def require_json_fields(*fields):
    def decorator(fn):
        ...
        def wrapper(*args, **kwargs):
            ...
            if missing:
                return jsonify({'error': 'Missing JSON fields', 'missing': missing}), 400
            return fn(*args, **kwargs)
        return wrapper
    return decorator
    ...
    def decorator(fn):
        ...
        def wrapper(*args, **kwargs):
            if field_name not in request.files:
                return jsonify({'error': 'Missing file parameter', 'missing': field_name}), 400
            return fn(*args, **kwargs)
        return wrapper
    return decorator 