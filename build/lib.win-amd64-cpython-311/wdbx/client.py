...
class WDBX:
    ...
    _search_cache: LRUCache[
        Tuple[Tuple[Union[float, int], ...], int], List[Dict[str, Any]]
    ]
    auto_discovery: bool
    discovery_port: int
    ...
    shard_manager: Optional[
        Any
    ]  # Replace Any with actual ShardManager type if available
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """
        logger.info(
            f"Initializing WDBX client with vector_dimension={vector_dimension}, enable_plugins={enable_plugins}"
        )
        ...
        try:
            ...
            from wdbx.async_internal import (
                AsyncInternalClient as ConcreteAsyncClient,
            )  # type: ignore

            ActualAsyncClient = ConcreteAsyncClient
            logger.info(
                "Successfully loaded ConcreteAsyncClient (AsyncInternalClient)."
            )
        except ImportError:
            logger.warning(
                "Concrete underlying async WDBX client (AsyncInternalClient) not found. "
                "Using a generic placeholder. Client functionality will be impaired."
            )
            ActualAsyncClient = (
                lambda **_kwargs: object()
            )  # A callable that returns a dummy object
        ...
        if self.shards:
            ...
            try:
                ...
                for node, cfg in self.shards.items():
                    ...
                    if hasattr(
                        shard_client_instance, "initialize"
                    ) and asyncio.iscoroutinefunction(shard_client_instance.initialize):
                ...
                logging.error(
                    f"Error initializing shards: {e}. Sharding may be impaired."
                )
                logger.error(
                    f"Error initializing shards: {e}. Sharding may be impaired."
                )
            ...
            logger.info(
                f"Auto-discovery enabled. Discovery port: {self.discovery_port}, RPC port: {self.rpc_port}"
            )
            try:
                self.discovery = NodeDiscovery(
                    service_port=self.rpc_port or 0, discovery_port=self.discovery_port
                )
                ...
                logging.error(
                    "ShardManager or NodeDiscovery not found for auto-discovery. Sharding disabled."
                )
                logger.error(
                    "ShardManager or NodeDiscovery not found for auto-discovery. Sharding disabled."
                )
                ...
                logging.error(
                    f"Error during auto-discovery/sharding setup: {e}. Sharding may be impaired."
                )
                logger.error(
                    f"Error during auto-discovery/sharding setup: {e}. Sharding may be impaired."
                )
    ...
    def __exit__(
        ...
        exc_tb: Optional[
            Any
        ],  # Actually TracebackType, but Any for simplicity if not importing `types`
    ) -> None:
        """
        ...
        with RPC_CALL_LATENCY.labels(method=method_name).time():
            try:
                if self._thread_safe and self._lock:
                    with self._lock:
                        logger.debug(
                            f"Executing async method '{method_name}' with lock."
                        )
                        result = asyncio.run(coro)
                else:
                    logger.debug(
                        f"Executing async method '{method_name}' without lock."
                    )
                    result = asyncio.run(coro)
                ...
                raise WDBXError(
                    f"Circuit breaker for '{method_name}' is open. The service may be temporarily unavailable or overloaded."
                ) from cb_err
            ...
            #     raise TimeoutError(f"Operation '{method_name}' timed out. The server might be busy or unresponsive.") from timeout_err
                ...
                raise WDBXError(
                    f"Async operation '{method_name}' failed with an unexpected error: {e}"
                ) from e
    ...
    def initialize(self) -> None:
        ...
        if not hasattr(self._client, "initialize") or not asyncio.iscoroutinefunction(
            self._client.initialize
        ):
            logger.warning(
                "Underlying client does not have a valid async initialize method. Skipping actual backend initialization."
            )
            return
    ...
    def shutdown(self) -> None:
        ...
        if hasattr(self._client, "shutdown") and asyncio.iscoroutinefunction(
            ...
            try:
                ...
                logger.warning(
                    f"Error during WDBX client shutdown: {e}. Attempting to proceed."
                )
        else:
            logger.warning(
                "Underlying client does not have a valid async shutdown method."
            )
        ...
        if self.shard_clients:
            ...
            for node_name, shard_client_instance in self.shard_clients.items():
                if hasattr(
                    shard_client_instance, "shutdown"
                ) and asyncio.iscoroutinefunction(shard_client_instance.shutdown):
                    try:
                        ...
                        logger.warning(
                            f"Error shutting down shard client {node_name}: {e}"
                        )
    ...
    def store(self, vector: List[float], metadata: Dict[str, Any]) -> int:
        ...
        if not isinstance(vector, list) or not all(
            isinstance(x, (float, int)) for x in vector
        ):
        ...
        logger.debug(
            f"Storing vector (size: {len(vector)}) with metadata (keys: {list(metadata.keys())})"
        )
        if not hasattr(
            self._client, "vector_store_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "vector_store_async")
        ):
            logger.error(
                "Underlying client does not support async method 'vector_store_async'."
            )
    ...
    def search(self, vector: List[float], limit: int = 10) -> List[Dict[str, Any]]:
        ...
        if cached_result is not None:
            logger.debug(
                f"Search cache hit for vector (size: {len(vector)}), limit: {limit}"
            )
            return cached_result

        logger.debug(
            f"Search cache miss for vector (size: {len(vector)}), limit: {limit}. Querying backend."
        )
        if not hasattr(
            self._client, "vector_search_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "vector_search_async")
        ):
            logger.error(
                "Underlying client does not support async method 'vector_search_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'vector_search_async'."
            )

        result: List[Dict[str, Any]] = self._run_async(
            self._client.vector_search_async(vector, limit=limit)
        )
        ...
        if self._thread_safe and self._lock:
            ...
            self._search_cache[cache_key] = result
        logger.info(
            f"Search returned {len(result)} results for vector (size: {len(vector)}), limit: {limit}"
        )
        return result
    ...
    def bulk_store(self, items: List[Tuple[List[float], Dict[str, Any]]]) -> List[int]:
        ...
        logger.info(
            f"Bulk store completed for {len(items)} items. Result IDs: {results}"
        )
        return results
    ...
    def bulk_search(
        ...
        logger.info(
            f"Bulk searching for {len(vectors)} query vectors with limit {limit}."
        )
        ...
        for v_idx, v_query in enumerate(vectors):
            if not all(isinstance(x, (float, int)) for x in v_query):
                raise TypeError(
                    f"Query vector at index {v_idx} must be a list of numbers."
                )
    ...
    def delete(self, vector_id: int) -> bool:
        ...
        if deleted:
            ...
            logger.warning(
                f"Failed to delete vector ID: {vector_id} or vector not found."
            )
        return deleted
    ...
    def update_metadata(self, vector_id: int, metadata: Dict[str, Any]) -> bool:
        ...
        logger.info(
            f"Updating metadata for vector ID: {vector_id} with keys: {list(metadata.keys())}"
        )
    ...
    def flush(self) -> None:
        ...
        """
        logger.warning(
            "Executing FLUSH operation. This will remove ALL vectors and data."
        )
    ...
    def create_block(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Creating blockchain block with metadata keys: {list(metadata.keys())}"
        )
        block_info = self._run_async(self._client.block_create_async(metadata))  # type: ignore
        logger.info(
            f"Blockchain block created successfully: {block_info.get('id', 'N/A')}"
        )
        return block_info
    ...
    def validate_block(self, block_id: Union[int, str]) -> bool:
        ...
        logger.info(
            f"Block {block_id} validation result: {'valid' if is_valid else 'invalid'}"
        )
        return is_valid
    ...
    def list_blocks(self, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        if not hasattr(
            self._client, "block_list_async"
        ) or not asyncio.iscoroutinefunction(getattr(self._client, "block_list_async")):
            logger.error(
                "Underlying client does not support async method 'block_list_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'block_list_async'."
            )
        blocks = self._run_async(
            self._client.block_list_async(limit=limit, offset=offset)
        )
    ...
    def get_block(self, block_id: Union[int, str]) -> Optional[Dict[str, Any]]:
        ...
        if not hasattr(
            self._client, "block_get_async"
        ) or not asyncio.iscoroutinefunction(getattr(self._client, "block_get_async")):
            logger.error(
                "Underlying client does not support async method 'block_get_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'block_get_async'."
            )
        try:
            ...
            if (
                "not found" in error_message_lower
                or "does not exist" in error_message_lower
                or "no such block" in error_message_lower
            ):
    ...
    def begin_transaction(self) -> Any:
        ...
        if not hasattr(
            self._client, "transaction_begin_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "transaction_begin_async")
        ):
            logger.error(
                "Underlying client does not support async method 'transaction_begin_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'transaction_begin_async'."
            )
    ...
    def commit_transaction(self, transaction_id: Any = None) -> None:
        ...
        """
        logger.info(
            f"Attempting to commit transaction: {transaction_id or 'current/implicit'}"
        )
        if not hasattr(
            self._client, "transaction_commit_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "transaction_commit_async")
        ):
            logger.error(
                "Underlying client does not support async method 'transaction_commit_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'transaction_commit_async'."
            )
        if transaction_id:
            self._run_async(
                self._client.transaction_commit_async(transaction_id=transaction_id)
            )
        else:
            self._run_async(self._client.transaction_commit_async())
        logger.info(
            f"Transaction committed successfully: {transaction_id or 'current/implicit'}"
        )
    ...
    def rollback_transaction(self, transaction_id: Any = None) -> None:
        ...
        """
        logger.info(
            f"Attempting to roll back transaction: {transaction_id or 'current/implicit'}"
        )
        if not hasattr(
            self._client, "transaction_rollback_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "transaction_rollback_async")
        ):
            logger.error(
                "Underlying client does not support async method 'transaction_rollback_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'transaction_rollback_async'."
            )
        if transaction_id:
            self._run_async(
                self._client.transaction_rollback_async(transaction_id=transaction_id)
            )
        else:
    ...
    def transaction(self) -> Iterator["WDBX"]:
        ...
        )  # Changed from "Beginning transaction..."
        tx_context = (
            self.begin_transaction()
        )  # tx_context might be used by commit/rollback if backend needs it
        try:
            yield self
            logger.info(f"Committing transaction (context: {tx_context})...")
            self.commit_transaction(
                transaction_id=tx_context if tx_context is not None else None
            )  # Pass if not None
    ...
    def schedule_self_update(
        ...
        """
        logger.info(
            f"Scheduling self-update from Git every {interval}s. Repo: {repo_dir}, Modules: {module_paths}"
        )
        # AdvancedUpdater is instantiated directly, its methods are synchronous
        adv = AdvancedUpdater(
            repo_url=getattr(self, "wdbx_repo_url", None)
        )  # Assuming repo_url can be an attribute
    ...
    def git_update(
        ...
        """
        logger.info(
            f"Performing immediate Git update. Local dir: {local_dir}, Modules: {module_paths}"
        )
    ...
    def rollback_update(
        ...
        """
        logger.info(
            f"Rolling back update for file: {file_path}. Backup: {backup_file or 'latest'}"
        )
    ...
    def ai_update(
        ...
        """
        logger.info(
            f"Performing AI-driven update for file: {file_path}. Instruction: '{instruction[:50]}...'"
        )
        adv = AdvancedUpdater(repo_url=getattr(self, "wdbx_repo_url", None))
        adv.ai_update(
            file_path,
        ...
        for entry_point in importlib.metadata.entry_points().select(
            ...
            try:
                ...
                if not hasattr(plugin_module, "wdbx_plugin_load"):
                    logger.warning(
                        f"Plugin {name} missing 'wdbx_plugin_load'. Skipping."
                    )
                    ...
                    continue
                logger.info(
                    f"Loading plugin: {name} v{info.get('version','?')} by {info.get('author','?')}"
                )
        ...
        if hasattr(self._client, "load_async_plugin"):
            ...
            for entry_point in importlib.metadata.entry_points().select(
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to load async plugin {name}: {e}", exc_info=True
                    )
    ...
    def neural_backtrack(self, pattern: List[float], depth: int = 3) -> Dict[str, Any]:
        ...
        if not isinstance(pattern, list) or not all(
            isinstance(x, (float, int)) for x in pattern
        ):
        ...
        logger.info(
            f"Performing neural backtrack for pattern (size: {len(pattern)}), depth: {depth}"
        )
        if not hasattr(
            self._client, "neural_backtrack_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "neural_backtrack_async")
        ):
            logger.error(
                "Underlying client does not support async method 'neural_backtrack_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'neural_backtrack_async'."
            )
        results = self._run_async(
            self._client.neural_backtrack_async(pattern, depth=depth)
        )
        logger.info(
            f"Neural backtrack completed. Result keys: {list(results.keys()) if isinstance(results, dict) else 'N/A'}"
        )
        return results

    def detect_drift(
        self, threshold: float = 0.1, dataset_sample: Optional[List[List[float]]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if dataset_sample is not None:
            if not isinstance(dataset_sample, list) or not all(
                isinstance(vec, list) and all(isinstance(x, (float, int)) for x in vec)
                for vec in dataset_sample
            ):
                raise TypeError(
                    "Dataset sample for drift detection must be a list of lists of numbers."
                )

        logger.info(
            f"Detecting drift with threshold: {threshold}. Sample provided: {dataset_sample is not None}"
        )
        if not hasattr(
            self._client, "drift_detect_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "drift_detect_async")
        ):
            logger.error(
                "Underlying client does not support async method 'drift_detect_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'drift_detect_async'."
            )
        results = self._run_async(
            self._client.drift_detect_async(
                threshold=threshold, dataset_sample=dataset_sample
            )
        )
        logger.info(
            f"Drift detection completed. Drift detected: {results.get('drift_detected', 'N/A')}, Score: {results.get('score', 'N/A')}"
        )
        return results
    ...
    def create_persona(self, name: str, config: Dict[str, Any]) -> int:
        ...
        logger.info(
            f"Creating persona with name: '{name}', config keys: {list(config.keys())}"
        )
        if not hasattr(
            self._client, "persona_create_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "persona_create_async")
        ):
            logger.error(
                "Underlying client does not support async method 'persona_create_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'persona_create_async'."
            )
    ...
    def switch_persona(self, name: str) -> bool:
        ...
        if not hasattr(
            self._client, "persona_switch_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "persona_switch_async")
        ):
            logger.error(
                "Underlying client does not support async method 'persona_switch_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'persona_switch_async'."
            )
        ...
        logger.info(
            f"Switch persona to '{name}' {'succeeded' if success else 'failed'}."
        )
        return success
    ...
    def list_personas(self, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        if not hasattr(
            self._client, "persona_list_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "persona_list_async")
        ):
            logger.error(
                "Underlying client does not support async method 'persona_list_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'persona_list_async'."
            )
        personas = self._run_async(
            self._client.persona_list_async(limit=limit, offset=offset)
        )
        logger.info(f"Retrieved {len(personas)} personas.")
        return personas

    def filter_content(
        self, text: str, ruleset_id: Optional[Union[int, str]] = None
    ) -> str:
        """
        ...
        logger.info(
            f"Filtering content (length: {len(text)}). Ruleset ID: {ruleset_id or 'default'}"
        )
        if not hasattr(
            self._client, "content_filter_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "content_filter_async")
        ):
            logger.error(
                "Underlying client does not support async method 'content_filter_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'content_filter_async'."
            )
        filtered_text = self._run_async(
            self._client.content_filter_async(text, ruleset_id=ruleset_id)
        )
        logger.info(
            f"Content filtering completed. Original length: {len(text)}, Filtered length: {len(filtered_text)}"
        )
    ...
    def mitigate_bias(self, text: str, method: Optional[str] = None) -> str:
        ...
        logger.info(
            f"Mitigating bias in content (length: {len(text)}). Method: {method or 'default'}"
        )
        if not hasattr(
            self._client, "bias_mitigate_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "bias_mitigate_async")
        ):
            logger.error(
                "Underlying client does not support async method 'bias_mitigate_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'bias_mitigate_async'."
            )
        mitigated_text = self._run_async(
            self._client.bias_mitigate_async(text, method=method)
        )
        logger.info(
            f"Bias mitigation completed. Original length: {len(text)}, Mitigated length: {len(mitigated_text)}"
        )
    ...
    def start_http_server(
        ...
        if use_ssl:
            if not certfile or not keyfile:
                raise ValueError(
                    "Both certfile and keyfile are required when use_ssl is True."
                )
        ...
        if not hasattr(
            ...
            getattr(self._client, "http_server_start_async")
        ):
            logger.error(
                "Underlying client does not support async method 'http_server_start_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'http_server_start_async'."
            )
    ...
    def stop_http_server(self) -> None:
        ...
        if not hasattr(
            self._client, "http_server_stop_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "http_server_stop_async")
        ):
            logger.error(
                "Underlying client does not support async method 'http_server_stop_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'http_server_stop_async'."
            )
    ...
    def get_api_endpoints(self) -> List[Dict[str, Any]]:
        ...
        if not hasattr(
            self._client, "api_get_endpoints_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "api_get_endpoints_async")
        ):
            logger.error(
                "Underlying client does not support async method 'api_get_endpoints_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'api_get_endpoints_async'."
            )
    ...
    def set_api_auth_required(
        self, required: bool = True, endpoint_paths: Optional[List[str]] = None
    ) -> None:
        """
        ...
        if endpoint_paths is not None:
            if not isinstance(endpoint_paths, list) or not all(
                isinstance(p, str) for p in endpoint_paths
            ):
                raise TypeError("endpoint_paths must be a list of strings if provided.")

        target_endpoints = (
            "globally" if endpoint_paths is None else f"for endpoints: {endpoint_paths}"
        )
        logger.info(
            f"Setting API authentication required: {required} {target_endpoints}"
        )
        if not hasattr(
            self._client, "api_set_auth_required_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "api_set_auth_required_async")
        ):
            logger.error(
                "Underlying client does not support async method 'api_set_auth_required_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'api_set_auth_required_async'."
            )
        self._run_async(
            self._client.api_set_auth_required_async(
                required=required, endpoint_paths=endpoint_paths
            )
        )
        logger.info(
            f"API authentication requirement set to {required} {target_endpoints}."
        )
    ...
    def set_api_cors_origins(self, origins: List[str]) -> None:
        ...
        if not hasattr(
            self._client, "api_set_cors_origins_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "api_set_cors_origins_async")
        ):
            logger.error(
                "Underlying client does not support async method 'api_set_cors_origins_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'api_set_cors_origins_async'."
            )
    ...
    def enable_plugin(self, plugin_name: str) -> bool:
        ...
        if not hasattr(
            self._client, "plugin_enable_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "plugin_enable_async")
        ):
            logger.error(
                "Underlying client does not support async method 'plugin_enable_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'plugin_enable_async'."
            )
    ...
    def disable_plugin(self, plugin_name: str) -> bool:
        ...
        if not hasattr(
            self._client, "plugin_disable_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "plugin_disable_async")
        ):
            logger.error(
                "Underlying client does not support async method 'plugin_disable_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'plugin_disable_async'."
            )
    ...
    def reload_plugin(self, plugin_name: str) -> bool:
        ...
        if module and hasattr(module, "on_reload"):
            try:
                ...
                logger.error(
                    f"Failed to reload plugin {plugin_name}: {e}", exc_info=True
                )
                plugin["error"] = str(e)
                plugin["status"] = "error"
        return False
    ...
    def visualize_vectors(
        self, vectors: List[List[float]], method: str = "tsne", **kwargs: Any
    ) -> Dict[str, Any]:
        """
        ...
        """
        if not isinstance(vectors, list) or not all(
            isinstance(v, list) and all(isinstance(x, (float, int)) for x in v)
            for v in vectors
        ):
            raise TypeError(
                "Input for visualize_vectors must be a list of lists of numbers."
            )
        if not isinstance(method, str) or not method.strip():
            raise TypeError("Visualization method must be a non-empty string.")

        logger.info(
            f"Visualizing {len(vectors)} vectors using method: '{method}'. Additional args: {kwargs}"
        )
        if not hasattr(
            self._client, "vectors_visualize_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "vectors_visualize_async")
        ):
            logger.error(
                "Underlying client does not support async method 'vectors_visualize_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'vectors_visualize_async'."
            )
        visualization_data = self._run_async(
            self._client.vectors_visualize_async(vectors, method=method, **kwargs)
        )
        logger.info(
            f"Vector visualization generated using '{method}'. Result keys: {list(visualization_data.keys()) if isinstance(visualization_data, dict) else 'N/A'}"
        )
    ...
    def set_access_token(self, token: str) -> None:
        ...
        if not hasattr(
            self._client, "set_access_token_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "set_access_token_async")
        ):
            logger.error(
                "Underlying client does not support async method 'set_access_token_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'set_access_token_async'."
            )
        ...
        logger.info(
            "Access token set successfully (first few chars for verification if appropriate)."
        )  # Be cautious logging tokens
    ...
    def encrypt_data(self, data: Any, key_id: Optional[str] = None) -> str:
        ...
        if not hasattr(
            self._client, "encrypt_data_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "encrypt_data_async")
        ):
            logger.error(
                "Underlying client does not support async method 'encrypt_data_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'encrypt_data_async'."
            )
        encrypted_data_str = self._run_async(
            self._client.encrypt_data_async(data, key_id=key_id)
        )
        logger.info(
            f"Data encrypted successfully. Encrypted data length: {len(encrypted_data_str)}"
        )
    ...
    def decrypt_data(self, encrypted_data: str, key_id: Optional[str] = None) -> Any:
        ...
        logger.info(
            f"Decrypting data (length: {len(encrypted_data)}). Key ID: {key_id or 'default'}"
        )
        if not hasattr(
            self._client, "decrypt_data_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "decrypt_data_async")
        ):
            logger.error(
                "Underlying client does not support async method 'decrypt_data_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'decrypt_data_async'."
            )
        decrypted_data = self._run_async(
            self._client.decrypt_data_async(encrypted_data, key_id=key_id)
        )
    ...
    def generate_api_key(
        ...
        if expires_at is not None and not isinstance(expires_at, (str, int)):
            raise TypeError(
                "expires_at must be a string (ISO 8601) or integer (Unix timestamp) if provided."
            )

        logger.info(
            f"Generating API key for user_id: '{user_id}'. Permissions: {permissions}, Expires: {expires_at}"
        )
        if not hasattr(
            self._client, "api_key_generate_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "api_key_generate_async")
        ):
            logger.error(
                "Underlying client does not support async method 'api_key_generate_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'api_key_generate_async'."
            )
        key_info = self._run_async(
            self._client.api_key_generate_async(
                user_id, permissions=permissions, expires_at=expires_at
            )
        )
        logger.info(
            f"API key generated successfully for user_id: '{user_id}'. Key ID/Prefix: {key_info.get('id', key_info.get('key_prefix', 'N/A'))}"
        )
        return key_info
    ...
    def revoke_api_key(self, key_identifier: str) -> bool:
        ...
        if not isinstance(key_identifier, str) or not key_identifier.strip():
            raise TypeError(
                "API key identifier for revocation must be a non-empty string."
            )

        logger.info(
            f"Revoking API key with identifier/prefix: '{key_identifier[:10]}...'"
        )  # Log only a prefix
        if not hasattr(
            self._client, "api_key_revoke_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "api_key_revoke_async")
        ):
            logger.error(
                "Underlying client does not support async method 'api_key_revoke_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'api_key_revoke_async'."
            )
        ...
        logger.info(
            f"API key revocation for '{key_identifier[:10]}...' {'succeeded' if success else 'failed'}."
        )
        return success
    ...
    def assign_role(self, user_id: str, roles: List[str]) -> bool:
        ...
        if not hasattr(
            self._client, "user_assign_roles_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "user_assign_roles_async")
        ):  # Assuming plural roles
            logger.error(
                "Underlying client does not support async method for assigning roles."
            )
            raise NotImplementedError(
                "Underlying client does not support async method for assigning roles."
            )
        ...
        logger.info(
            f"Role assignment for user_id '{user_id}' {'succeeded' if success else 'failed'}."
        )
        return success
    ...
    def remove_role(self, user_id: str, roles: List[str]) -> bool:
        ...
        if not hasattr(
            self._client, "user_remove_roles_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "user_remove_roles_async")
        ):  # Assuming plural roles
            logger.error(
                "Underlying client does not support async method for removing roles."
            )
            raise NotImplementedError(
                "Underlying client does not support async method for removing roles."
            )
        ...
        logger.info(
            f"Role removal for user_id '{user_id}' {'succeeded' if success else 'failed'}."
        )
        return success
    ...
    def get_audit_logs(
        ...
        if since is not None and not isinstance(since, (str, int)):
            raise TypeError(
                "since filter must be a string (ISO 8601) or int (Unix timestamp) if provided."
            )
        if until is not None and not isinstance(until, (str, int)):
            raise TypeError(
                "until filter must be a string (ISO 8601) or int (Unix timestamp) if provided."
            )

        logger.info(
            f"Fetching audit logs with limit={limit}, offset={offset}, user_id={user_id}, action={action}, since={since}, until={until}"
        )
        if not hasattr(
            self._client, "audit_logs_get_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "audit_logs_get_async")
        ):
            logger.error(
                "Underlying client does not support async method 'audit_logs_get_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'audit_logs_get_async'."
            )
    ...
    def set_rate_limit(
        self, endpoint: str, limit: int, period_seconds: int, scope: str = "ip"
    ) -> bool:
        """
        ...
        ):  # Allow 0 for blocking? Backend dependent.
        ...
        logger.info(
            f"Setting rate limit for endpoint '{endpoint}': {limit} requests per {period_seconds}s, scope: {scope}"
        )
        if not hasattr(
            self._client, "rate_limit_set_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "rate_limit_set_async")
        ):
            logger.error(
                "Underlying client does not support async method 'rate_limit_set_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'rate_limit_set_async'."
            )
        success = self._run_async(
            self._client.rate_limit_set_async(
                endpoint, limit, period_seconds, scope=scope
            )
        )
        logger.info(
            f"Rate limit setting for endpoint '{endpoint}' {'succeeded' if success else 'failed'}."
        )
        return success
    ...
    def enable_cache(self, cache_name: Optional[str] = None) -> bool:
        ...
        if cache_name is not None and (
            not isinstance(cache_name, str) or not cache_name.strip()
        ):
        ...
        if not hasattr(
            self._client, "cache_enable_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "cache_enable_async")
        ):
            logger.error(
                "Underlying client does not support async method 'cache_enable_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'cache_enable_async'."
            )
        success = self._run_async(
            self._client.cache_enable_async(cache_name=cache_name)
        )
        logger.info(
            f"Cache enabling for '{cache_name or 'default/global'}' {'succeeded' if success else 'failed'}."
        )
        return success
    ...
    def disable_cache(self, cache_name: Optional[str] = None) -> bool:
        ...
        if not hasattr(
            self._client, "cache_disable_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "cache_disable_async")
        ):
            logger.error(
                "Underlying client does not support async method 'cache_disable_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'cache_disable_async'."
            )
        success = self._run_async(
            self._client.cache_disable_async(cache_name=cache_name)
        )
        logger.info(
            f"Cache disabling for '{cache_name or 'default/global'}' {'succeeded' if success else 'failed'}."
        )
        return success
    ...
    def get_cache_stats(self, cache_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not hasattr(
            self._client, "cache_get_stats_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "cache_get_stats_async")
        ):
            logger.error(
                "Underlying client does not support async method 'cache_get_stats_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'cache_get_stats_async'."
            )
        stats = self._run_async(
            self._client.cache_get_stats_async(cache_name=cache_name)
        )
    ...
    def backup_database(self, path: str, strategy: str = "full") -> bool:
        ...
        if not hasattr(
            self._client, "database_backup_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "database_backup_async")
        ):
            logger.error(
                "Underlying client does not support async method 'database_backup_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'database_backup_async'."
            )
        success = self._run_async(
            self._client.database_backup_async(path, strategy=strategy)
        )
        logger.info(
            f"Database backup to '{path}' {'succeeded' if success else 'failed'}."
        )
        return success
    ...
    def restore_database(self, path: str, strategy: str = "full") -> bool:
        ...
        logger.warning(
            f"Restoring database from '{path}' using strategy: '{strategy}'. This is a potentially destructive operation."
        )
        if not hasattr(
            self._client, "database_restore_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "database_restore_async")
        ):
            logger.error(
                "Underlying client does not support async method 'database_restore_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'database_restore_async'."
            )
        success = self._run_async(
            self._client.database_restore_async(path, strategy=strategy)
        )
        logger.info(
            f"Database restore from '{path}' {'succeeded' if success else 'failed'}."
        )
        return success
    ...
    def get_database_version(self) -> str:
        ...
        if not hasattr(
            self._client, "database_get_version_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "database_get_version_async")
        ):
            logger.error(
                "Underlying client does not support async method 'database_get_version_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'database_get_version_async'."
            )
    ...
    def supports_feature(self, feature_name: str) -> bool:
        ...
        if not hasattr(
            self._client, "feature_supports_async"
        ) or not asyncio.iscoroutinefunction(
            getattr(self._client, "feature_supports_async")
        ):
            logger.error(
                "Underlying client does not support async method 'feature_supports_async'."
            )
            raise NotImplementedError(
                "Underlying client does not support async method 'feature_supports_async'."
            )
        ...
        logger.info(
            f"Feature '{feature_name}' is {'supported' if supported else 'not supported'}."
        )
        return supported
    ...
    def shard_passthrough(
        self, shard_id: str, method_name: str, *args: Any, **kwargs: Any
    ) -> Any:
        """
        ...
        """
        logger.info(
            f"Executing passthrough command '{method_name}' on shard '{shard_id}' with args: {args}, kwargs: {kwargs}"
        )
        if shard_id not in self.shard_clients:
            logger.error(f"Shard ID '{shard_id}' not found for passthrough operation.")
            raise KeyError(
                f"Shard ID '{shard_id}' not found. Available shards: {list(self.shard_clients.keys())}"
            )

        shard_client = self.shard_clients[shard_id]
        if not hasattr(shard_client, method_name):
            logger.error(
                f"Method '{method_name}' not found on shard client for shard '{shard_id}'."
            )
            raise AttributeError(
                f"Method '{method_name}' not found on client for shard '{shard_id}'."
            )

        method_to_call = getattr(shard_client, method_name)
        if not asyncio.iscoroutinefunction(method_to_call):
            logger.error(
                f"Method '{method_name}' on shard client for shard '{shard_id}' is not an async coroutine function."
            )
            raise AttributeError(
                f"Method '{method_name}' on shard client for shard '{shard_id}' is not an async coroutine function."
            )
        ...
        try:
            ...
            logger.info(
                f"Passthrough command '{method_name}' on shard '{shard_id}' completed successfully."
            )
            return result
        except (
            Exception
        ) as e:  # Catches WDBXError from _run_async or other direct exceptions
            logger.error(
            ...
            if not isinstance(e, WDBXError):
                raise WDBXError(
                    f"Shard passthrough command '{method_name}' on '{shard_id}' failed: {e}"
                ) from e
            raise
