...
class AsyncLylexDB:
        ...
        **kwargs: Any,
        ...
        if (
            ...
            if self.vector_dimension != openai_ada_dim:
                logger.warning(
                    f"AsyncLylexDB initialized with vector_dimension={self.vector_dimension}, "
    ...
    async def _get_vector(self, prompt: str) -> List[float]:
        """
        ...
        if self.embedding_handler and os.getenv("OPENAI_API_KEY"):
            try:
                ...
                vector = await loop.run_in_executor(
                    None, lambda: self.embedding_handler.embed([prompt])[0]
                )

                if len(vector) != self.vector_dimension:
                    logger.error(
                        f"OpenAI EmbeddingHandler returned vector of dimension {len(vector)}, "
                        f"but AsyncLylexDB is configured for {self.vector_dimension}. Falling back to zero-vector."
                    )
        ...
        if self.model_handler and hasattr(self.model_handler, "get_embedding"):
            try:
                loop = asyncio.get_event_loop()
                vector = await loop.run_in_executor(
                    None, self.model_handler.get_embedding, prompt
                )
                if len(vector) != self.vector_dimension:
                    logger.error(
    ...
    async def store_interaction(
        ...
        if hasattr(self.client, "distributed_store") and getattr(
            self.client, "shard_manager", None
        ):
            return await self.client.distributed_store(prompt, vector, metadata)
        return await self.client.store(vector, metadata)

    async def bulk_store_interactions(
    ...
    async def broadcast_search_interactions(
        ...
        """
        vector = await self._get_vector(prompt)
        if hasattr(self.client, "broadcast_search") and getattr(
            self.client, "shard_clients", None
        ):
    ...
    async def update_interaction_metadata(
        self, vector_id: int, metadata: Dict[str, Any]
    ) -> bool:
        """
