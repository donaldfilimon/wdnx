...
def generate_ollama():
    ...
    try:
        response = requests.post(OLLAMA_API_URL, json=data, stream=data.get("stream", False))
        ...
        if data.get("stream", False):
            ...
            logger.info(f"Ollama streaming response initiated for model: {data.get('model')}")
            return jsonify({"message": "Streaming initiated, but this proxy doesn't fully support it. Check Ollama directly."})
        else:
            logger.info(f"Ollama non-streaming response successful for model: {data.get('model')}")
            return jsonify(response.json())
    except requests.exceptions.RequestException as e:
        logger.error(f"Ollama request failed for model {data.get('model')}: {e}", exc_info=True)
        raise # Reraise to be caught by centralized error handler, will produce 502 or similar
    except Exception as e:
        logger.error(f"Ollama generic error for model {data.get('model')}: {e}", exc_info=True)
        raise
