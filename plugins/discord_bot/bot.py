...
from encryption_utils import generate_key, encrypt_data, decrypt_data
...
def setup_slash_commands():
    ...
    @bot.tree.command(name='scrape', description='Scrape a URL and return page title and first paragraph')
    ...
    async def slash_decrypt(interaction: discord.Interaction, key_hex: str, nonce_hex: str, ciphertext_hex: str):
        try:
            key = bytes.fromhex(key_hex)
