...
def tune_api():
    ...
    try:
        ...
        return jsonify({
            ...
            "best_trial": {"value": study.best_trial.value, "params": study.best_trial.params}
        })
    except Exception as e:
        logger.error(f"ML tuning error for model {cfg.get('model_name')}, study {cfg.get('study_name')}: {e}", exc_info=True)
        raise 