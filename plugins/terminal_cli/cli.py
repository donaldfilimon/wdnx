...
@click.option('--workers', type=int, default=None, help='Number of worker threads for parallel processing.')
...
@click.option('--anchor-hash', 'anchor_hash_flag', is_flag=True, help='Anchor the encrypted HTML hash to the blockchain.')
@with_appcontext
def convert_pdf_cli(input_pdf, output_html, workers, skip_vision, extract_tables, encrypt_html, anchor_hash_flag):
    """
...
@click.option('--repo-dir', 'repo_dir', type=click.Path(), default='.', help='Repository directory')
...
def self_update(wdbx_flag, lylex_flag, interval, repo_dir, module_paths):
    ...
    if wdbx_flag:
        from app import wdbx
        wdbx.schedule_self_update(interval=interval, repo_dir=repo_dir, module_paths=list(module_paths) or None)
        click.echo(f'Scheduled WDBX self-update every {interval} minutes for {repo_dir}')
    if lylex_flag:
        from app import lylex_db
        lylex_db.schedule_self_update(interval=interval, repo_dir=repo_dir, module_paths=list(module_paths) or None)
...
@click.option('--local-dir', 'local_dir', type=click.Path(), default='.', help='Local repository directory')
