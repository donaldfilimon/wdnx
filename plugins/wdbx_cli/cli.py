...
def handle_wdbx_call(ctx, func, success_message, *args, **kwargs):
    ...
    try:
        ...
        if ctx.obj.get('JSON_OUTPUT'):
            click.echo(json.dumps({'status': 'success', 'result': result}, indent=2))
        else:
        ...
        error_info = {'status': 'error', 'message': str(e), 'details': getattr(e, 'details', None)}
        if ctx.obj.get('JSON_OUTPUT'):
        ...
        error_info = {'status': 'error', 'message': "An unexpected error occurred.", 'details': str(e)}
        if ctx.obj.get('JSON_OUTPUT'):
            click.echo(json.dumps(error_info, indent=2))
        else:
...
@click.option('--host', envvar='WDBX_HOST', help="WDBX server host. Can also be set via WDBX_HOST env var.")
@click.option('--port', envvar='WDBX_PORT', type=int, help="WDBX server port. Can also be set via WDBX_PORT env var.")
@click.option('--token', envvar='WDBX_TOKEN', help="JWT token for authentication. Can also be set via WDBX_TOKEN env var.")
...
@click.option('--metadata', 'metadata_str', default='{}', help='JSON string for metadata. Example: '{"key": "value"}'')
@click.pass_context
...
@click.argument('vector', nargs=-1, type=float, required=True)
@click.option('--limit', default=10, type=int, show_default=True, help='Maximum number of results to return.')
@click.pass_context
...
def bulk_store(ctx, items_json_or_file):
    ...
    try:
        ...
        click.echo(f"Error: Invalid JSON format in input or file '{items_json_or_file}'.", err=True)
        ctx.exit(1)
...
@click.option('--limit', default=10, type=int, show_default=True, help='Max results per query vector.')
@click.pass_context
def bulk_search(ctx, vectors_json_or_file, limit):
    ...
    try:
        ...
        click.echo(f"Error: Invalid JSON format in input or file '{vectors_json_or_file}'.", err=True)
        ctx.exit(1)
    except FileNotFoundError:
...
@click.argument('vector_id', type=int)
@click.option('--metadata', 'metadata_str', required=True, help='JSON string for new metadata. Example: '{"key": "new_value"}'')
@click.pass_context
def update_metadata(ctx, vector_id, metadata_str):
    ...
    client = WDBX(host=ctx.obj.get('HOST'), port=ctx.obj.get('PORT'), token=ctx.obj.get('TOKEN'))
    handle_wdbx_call(ctx, client.update_metadata, f"Metadata updated for vector ID {vector_id}.", vector_id, metadata)
...
def get_metadata(ctx, vector_id):
    ...
    """
    client = WDBX(host=ctx.obj.get('HOST'), port=ctx.obj.get('PORT'), token=ctx.obj.get('TOKEN'))
    handle_wdbx_call(ctx, client.get_metadata, f"Metadata retrieved for vector ID {vector_id}.", vector_id)
...
def ping(ctx):
    ...
    try:
        ...
        if ctx.obj.get('JSON_OUTPUT'):
            click.echo(json.dumps({'status': 'success', 'result': {'alive': alive, 'message': message}}))
        else:
            click.echo(message)
...
@click.option('--confirm', is_flag=True, prompt='Are you sure you want to flush all vectors? This cannot be undone.', help="Confirm the flush operation.")
@click.pass_context
...
def list_shards(ctx):
    ...
    try:
        ...
        if ctx.obj.get('JSON_OUTPUT'):
            click.echo(json.dumps({'status': 'success', 'result': shards}, indent=2))
        else:
            if shards:
        ...
        error_info = {'status': 'error', 'message': "Failed to list shards.", 'details': str(e)}
        if ctx.obj.get('JSON_OUTPUT'):
            click.echo(json.dumps(error_info, indent=2))
        else:
